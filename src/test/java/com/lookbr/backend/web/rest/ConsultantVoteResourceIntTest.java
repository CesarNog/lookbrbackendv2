...
public class ConsultantVoteResourceIntTest {
    ...
    @Autowired
    private ConsultantVoteSearchRepository consultantVoteSearchRepository;
    ...
    public void initTest() {
        consultantVoteSearchRepository.deleteAll();
    ...
    public void createConsultantVote() throws Exception {
        ...
        // Validate the ConsultantVote in Elasticsearch
        ConsultantVote consultantVoteEs = consultantVoteSearchRepository.findOne(testConsultantVote.getId());
        assertThat(consultantVoteEs).isEqualToIgnoringGivenFields(testConsultantVote);
    }
    ...
    public void deleteConsultantVote() throws Exception {
        ...
        boolean consultantVoteExistsInEs = consultantVoteSearchRepository.exists(consultantVote.getId());
        assertThat(consultantVoteExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchConsultantVote() throws Exception {
        // Initialize the database
        consultantVoteRepository.saveAndFlush(consultantVote);
        consultantVoteSearchRepository.save(consultantVote);

        // Search the consultantVote
        restConsultantVoteMockMvc.perform(get("/api/_search/consultant-votes?query=id:" + consultantVote.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(consultantVote.getId().intValue())))
            .andExpect(jsonPath("$.[*].consultantProfilePhotoUrl").value(hasItem(DEFAULT_CONSULTANT_PROFILE_PHOTO_URL.toString())));
