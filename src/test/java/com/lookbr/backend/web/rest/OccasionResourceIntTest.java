...
public class OccasionResourceIntTest {
    ...
    @Autowired
    private OccasionSearchRepository occasionSearchRepository;
    ...
    public void initTest() {
        occasionSearchRepository.deleteAll();
    ...
    public void createOccasion() throws Exception {
        ...
        // Validate the Occasion in Elasticsearch
        Occasion occasionEs = occasionSearchRepository.findOne(testOccasion.getId());
        assertThat(occasionEs).isEqualToIgnoringGivenFields(testOccasion);
    }
    ...
    public void deleteOccasion() throws Exception {
        ...
        boolean occasionExistsInEs = occasionSearchRepository.exists(occasion.getId());
        assertThat(occasionExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchOccasion() throws Exception {
        // Initialize the database
        occasionRepository.saveAndFlush(occasion);
        occasionSearchRepository.save(occasion);

        // Search the occasion
        restOccasionMockMvc.perform(get("/api/_search/occasions?query=id:" + occasion.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(occasion.getId().intValue())))
            .andExpect(jsonPath("$.[*].page").value(hasItem(DEFAULT_PAGE)));
