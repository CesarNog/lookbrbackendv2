...
public class InspirationResourceIntTest {
    ...
    @Autowired
    private InspirationSearchRepository inspirationSearchRepository;
    ...
    public void initTest() {
        inspirationSearchRepository.deleteAll();
    ...
    public void createInspiration() throws Exception {
        ...
        // Validate the Inspiration in Elasticsearch
        Inspiration inspirationEs = inspirationSearchRepository.findOne(testInspiration.getId());
        assertThat(inspirationEs).isEqualToIgnoringGivenFields(testInspiration);
    }
    ...
    public void deleteInspiration() throws Exception {
        ...
        boolean inspirationExistsInEs = inspirationSearchRepository.exists(inspiration.getId());
        assertThat(inspirationExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchInspiration() throws Exception {
        // Initialize the database
        inspirationRepository.saveAndFlush(inspiration);
        inspirationSearchRepository.save(inspiration);

        // Search the inspiration
        restInspirationMockMvc.perform(get("/api/_search/inspirations?query=id:" + inspiration.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(inspiration.getId().intValue())))
            .andExpect(jsonPath("$.[*].consultantName").value(hasItem(DEFAULT_CONSULTANT_NAME.toString())))
