...
describe('Closet e2e test', () => {
    ...
    it('should load Closets', () => {
        navBarPage.goToEntity('closet');
        closetComponentsPage = new ClosetComponentsPage();
        expect(closetComponentsPage.getTitle()).toMatch(/lookbrbackendv2App.closet.home.title/);
    ...
    it('should load create Closet dialog', () => {
        ...
        expect(closetDialogPage.getModalTitle()).toMatch(/lookbrbackendv2App.closet.home.createOrEditLabel/);
...
export class ClosetComponentsPage {
    ...
    title = element.all(by.css('jhi-closet div h2 span')).first();
