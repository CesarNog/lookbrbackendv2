...
describe('Timeline e2e test', () => {
    ...
    it('should load Timelines', () => {
        navBarPage.goToEntity('timeline');
        timelineComponentsPage = new TimelineComponentsPage();
        expect(timelineComponentsPage.getTitle()).toMatch(/lookbrbackendv2App.timeline.home.title/);
    ...
    it('should load create Timeline dialog', () => {
        ...
        expect(timelineDialogPage.getModalTitle()).toMatch(/lookbrbackendv2App.timeline.home.createOrEditLabel/);
...
export class TimelineComponentsPage {
    ...
    title = element.all(by.css('jhi-timeline div h2 span')).first();
